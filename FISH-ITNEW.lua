...
-- ======= Instant Fish Mode: Ultra Integration =======
-- mode variable: "Normal" or "Ultra Fast"
local InstantFishMode = "Normal" -- default

-- Ultra mode state
local ultraRunning = false
local ultraSpamErrorCount = 0
local ultraSpamErrorLimit = 10
local ultraSpamBaseDelay = 0.03
local ultraJitterMax = 0.01
local ultraFishCheckTimeout = 6 -- seconds
local ultraFishHandlerConnection = nil

local function ultraRandJitter()
    return math.random() * ultraJitterMax
end

local function ultraForceResetFishing()
    pcall(function()
        if RFCancelFishingInputs then
            for i = 1,2 do safeInvoke(RFCancelFishingInputs) end
        end
    end)
    ...
    -- update lastFishTime used by watchdog
    lastFishTime = tick()
end

local function ultraDoChargeAndRequest()
    if RFChargeFishingRod then safeInvoke(RFChargeFishingRod, 2) end
    task.wait(0.02 + ultraRandJitter()*0.4)
    if RFRequestFishingMinigameStarted then safeInvoke(RFRequestFishingMinigameStarted, -1.25, 1) end
end

local function ultraSpamCompletedLoop()
    while ultraRunning do
        local ok = safeFire(REFishingCompleted)
        if not ok then
            ultraSpamErrorCount = ultraSpamErrorCount + 1
        else
            ultraSpamErrorCount = 0
        end

        if ultraSpamErrorCount >= ultraSpamErrorLimit then
            ultraSpamErrorCount = 0
            ultraForceResetFishing()
            task.wait(0.25 + math.random()*0.25)
        else
            task.wait(ultraSpamBaseDelay + ultraRandJitter())
        end
    end
end

local function setupUltraFishCaughtHandler()
    if not REFishCaught then return end
    if ultraFishHandlerConnection and ultraFishHandlerConnection.Connected then return end
    ultraFishHandlerConnection = REFishCaught.OnClientEvent:Connect(function(...)
        lastFishTime = tick()
        if ultraRunning then
            task.wait(0.03 + ultraRandJitter()*0.5)
            pcall(ultraDoChargeAndRequest)
        end
    end)
end

local function teardownUltraHandlers()
    if ultraFishHandlerConnection then
        pcall(function() ultraFishHandlerConnection:Disconnect() end)
        ultraFishHandlerConnection = nil
    end
end
...
function StartUltraFish()
    ...
    ultraSpamErrorCount = 0
    lastFishTime = tick()
    -- stop normal if running
    pcall(function() if autoFishingToggle then autoFishingToggle:Set(false) end end)
    setupUltraFishCaughtHandler()
    task.spawn(ultraSpamCompletedLoop)
    -- initial kick
    pcall(function() ultraDoChargeAndRequest() end)
    print("[FISH-IT] Ultra Fast Fishing Started")
end

function StopUltraFish()
    ultraRunning = false
    ultraSpamErrorCount = 0
    teardownUltraHandlers()
    print("[FISH-IT] Ultra Fast Fishing Stopped")
end

-- Auto-reactivate Ultra on respawn if mode selected
player.CharacterAdded:Connect(function()
    task.wait(1)
    if InstantFishMode == "Ultra Fast" then
        -- small delay to allow remotes to be available
        task.wait(0.6)
        pcall(function() StartUltraFish() end)
    end
end)

-- Watchdog for Ultra (auto recovery)
task.spawn(function()
    while true do
        if ultraRunning then
            if tick() - lastFishTime > ultraFishCheckTimeout then
                ultraForceResetFishing()
            end
        end
        task.wait(1)
...
-- Add dropdown selector in UI (placed near Instant Fishing toggle)
Auto:Dropdown({
    Title = "Instant Fish Mode",
    Values = {"Normal", "Ultra Fast"},
    Value = "Normal",
    Multi = false,
    Callback = function(selected)
        InstantFishMode = selected
        if selected == "Normal" then
            -- stop ultra and start normal toggle
            pcall(StopUltraFish)
            pcall(function() if autoFishingToggle then autoFishingToggle:Set(true) end end)
        else
            -- stop normal and start ultra
            pcall(function() if autoFishingToggle then autoFishingToggle:Set(false) end end)
            StartUltraFish()
        end
    end
})
-- ======= End Ultra Integration =======
