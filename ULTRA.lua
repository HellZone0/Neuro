...
-- ======= REPLACED: Instant Fish (ULTRA ONLY) =======
-- Single ultra-fast instant fish engine (replaces previous instant fish features)
do
    -- Ultra state
    local ultraRunning = false
    local ultraErrorCount = 0
    local ultraErrorLimit = 12
    local ultraBaseDelay = 0.0
    local ultraJitterMax = 0.001
    local ultraConn = nil

    local function ujitter() return math.random() * ultraJitterMax end
    ...
    local function ultraSpamLoop()
        while ultraRunning do
            ...
            if ultraErrorCount >= ultraErrorLimit then
                ...
                task.wait(ultraBaseDelay + ujitter())
            end
        end
    end
    ...
    -- Add a simple toggle in Main tab (Auto variable assumed)
    pcall(function()
        if Auto and Auto.Toggle then
            ...
            -- fallback minimal ScreenGui toggle
            if player and player:FindFirstChild("PlayerGui") and not player.PlayerGui:FindFirstChild("FISHITUltraToggle") then
                local screenGui = Instance.new("ScreenGui")
                screenGui.Name = "FISHITUltraToggle"
                screenGui.ResetOnSpawn = false
                screenGui.Parent = player.PlayerGui

                local btn = Instance.new("TextButton")
                btn.Size = UDim2.new(0,160,0,32)
                btn.Position = UDim2.new(0,8,0,200)
                btn.Text = "Start Instant Ultra"
                btn.Parent = screenGui
                btn.MouseButton1Click:Connect(function()
                    if not ultraRunning then
                        StartUltraFish()
                        btn.Text = "Stop Instant Ultra"
                    else
                        StopUltraFish()
                        btn.Text = "Start Instant Ultra"
                    end
