...
-- SCAN FUNCTION SIMPLE
local function scanFish()
    local Items = game:GetService("ReplicatedStorage"):FindFirstChild("Items")
    if not Items then return end
    
    fishTiers = {Common={}, Uncommon={}, Rare={}, Epic={}, Legendary={}, Mythic={}, SECRET={}}
    
    for _, item in pairs(Items:GetDescendants()) do
        if item:IsA("ModuleScript") then
            local success, data = pcall(require, item)
            if success and data and data.Data and data.Data.Id and data.Data.Tier then
                local tierName = ({[1]="Common",[2]="Uncommon",[3]="Rare",[4]="Epic",[5]="Legendary",[6]="Mythic",[7]="SECRET"})[data.Data.Tier]
                if tierName then
                    table.insert(fishTiers[tierName], data.Data.Id)
                end
            end
        end
    end
...
local function handleFish(fishId, weightData, itemData, isNew)
    ...
    if mutation and table.find(selectedMutations, mutation) then
        REFavoriteItem:FireServer(itemData and itemData.InventoryItem and itemData.InventoryItem.UUID or fishId)
        print("⭐ Mutation: " .. fishId .. " (" .. mutation .. ")")
    end
end
...
local function createNormalEmbed(fishName, playerName, tierName, tierColor, tierEmoji, weight, mutationStatus, fishPrice)
    return {
        ...
        footer = {
            text = "Zone Community • " .. os.date("%m/%d/%Y %I:%M %p")
        },
        timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
    }
end
...
local function testWebhookConnection()
    ...
    local payload = {
        username = "ZoneCommunity",
        embeds = {
            {
                title = "✅ Webhook Connected",
                description = "ZoneCommunity webhook system is now connected and ready!",
                color = 0x00FF00,
                fields = {
                    {
                        name = "**Status**",
                ...
                footer = {
                    text = "ZoneCommunity • " .. os.date("%m/%d/%Y %I:%M %p")
                },
                timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
            }
        }
