...
-- ======= FISH-IT ULTRA (Safe Append) =======
-- Adds "Instant Fish Mode" dropdown in Main -> Instant Fish integration
do
    local InstantFishMode = "Normal"

    local ultra = {}
    ultra.running = false
    ultra.errorCount = 0
    ultra.errorLimit = 10
    ultra.baseDelay = 0.03
    ultra.jitter = 0.01
    ultra.timeout = 6
    ultra.conn = nil

    local function ujitter() return math.random() * ultra.jitter end

    local function ultra_force_reset()
        pcall(function()
    ...
    local function ultra_do_charge_request()
        pcall(function() if RFChargeFishingRod then safeInvoke(RFChargeFishingRod,2) end end)
        task.wait(0.02 + ujitter()*0.4)
        pcall(function() if RFRequestFishingMinigameStarted then safeInvoke(RFRequestFishingMinigameStarted,-1.25,1) end end)
    end

    local function ultra_spam_loop()
        while ultra.running do
            local ok = pcall(function() return safeFire(REFishingCompleted) end)
            if not ok then
                ultra.errorCount = ultra.errorCount + 1
            else
                ultra.errorCount = 0
            end

            if ultra.errorCount >= ultra.errorLimit then
                ultra.errorCount = 0
                ultra_force_reset()
                task.wait(0.25 + math.random()*0.25)
            else
                task.wait(ultra.baseDelay + ujitter())
            end
        end
    end

    local function ultra_setup_handler()
        if not REFishCaught then return end
        if ultra.conn and ultra.conn.Connected then return end
        ultra.conn = REFishCaught.OnClientEvent:Connect(function(...)
            lastFishTime = tick()
            if ultra.running then
                task.wait(0.03 + ujitter()*0.5)
                pcall(ultra_do_charge_request)
            end
        end)
    end

    local function ultra_teardown()
        if ultra.conn then
            pcall(function() ultra.conn:Disconnect() end)
            ultra.conn = nil
        end
    end

    function StartUltraFish()
        if ultra.running then return end
        ultra.running = true
        ultra.errorCount = 0
        lastFishTime = tick()
        -- ensure normal autoFishingToggle is off if available
        pcall(function() if type(autoFishingToggle) ~= 'nil' and autoFishingToggle.Set then autoFishingToggle:Set(false) end end)
        ultra_setup_handler()
        task.spawn(ultra_spam_loop)
        pcall(function() ultra_do_charge_request() end)
    ...
    -- auto-reactivate on respawn if dropdown still set to Ultra Fast
    pcall(function()
    ...
    task.spawn(function()
        while true do
            if ultra.running then
                if tick() - lastFishTime > ultra.timeout then
                    ultra_force_reset()
                end
            end
            task.wait(1)
    ...
    -- Add dropdown UI only if Auto (the Main tab object) exists
    pcall(function()
        if Auto and Auto.Dropdown then
            Auto:Dropdown({
                ...
                Callback = function(selected)
                    ...
                    if selected == "Normal" then
                        pcall(StopUltraFish)
                        -- re-enable original toggle if exists
                        pcall(function() if type(autoFishingToggle) ~= 'nil' and autoFishingToggle.Set then autoFishingToggle:Set(true) end end)
                    else
                        pcall(function() if type(autoFishingToggle) ~= 'nil' and autoFishingToggle.Set then autoFishingToggle:Set(false) end end)
                        pcall(StartUltraFish)
                    end
            ...
            -- fallback: create a simple ScreenGui toggle if Auto API missing
            -- (not creating here to avoid UI clash)
        end
    end)
end

-- ======= End FISH-IT ULTRA (Safe Append) =======
