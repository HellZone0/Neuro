...
local function forceResetFishing()
    ...
    resetTool()
    task.wait(0.1 + math.random()*0.05) -- random tiny delay
    doChargeAndRequest()
    lastFishTime=tick()
end

-- Spam completed loop (hybrid ultra)
local function spamCompletedLoop()
    while running do
        safeFire(REFishingCompleted)
        task.wait(0.005 + math.random()*0.01) -- micro-random delay
    end
end
...
local function equipToolLoop()
    while running do
        safeFire(REEquipToolFromHotbar,1)
        task.wait(1.5 + math.random()*0.3) -- sering tapi random
    end
end
...
autoFishingToggle=Auto:Toggle({
    Title="Auto Fishing",
    Type="Toggle",
    Desc="Hybrid Ultra-Aggressive Instant Fishing",
    Default=false,
    Callback=function(state)
        running=state
        if running then
