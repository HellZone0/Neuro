...
local function spamCompletedLoop()
    while running do
        ...
        task.wait(0.01 + math.random()*0.01) -- ultra cepat & random
    end
end
...
local function equipToolLoop()
    while running do
        safeFire(REEquipToolFromHotbar,1)
        task.wait(1.5) -- cukup sering tapi tidak spam berlebihan
    end
end
...
REFishCaught.OnClientEvent:Connect(function(fishName,fishData)
    ...
    if running then
        task.spawn(function()
            task.wait(0.01 + math.random()*0.02) -- ultra cepat random
            doChargeAndRequest()
        end)
    end
end)
...
autoFishingToggle=Auto:Toggle({
    Title="Auto Fishing",
    Type="Toggle",
    Desc="Ultra-Aggressive Instant Fishing",
    Default=false,
    Callback=function(state)
        running=state
        if running then
