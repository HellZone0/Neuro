...
local fishCaughtConnection = nil -- NEW: Untuk mengelola koneksi event

-- [Fungsi safeFire dan safeInvoke tetap sama, saya tidak memasukkannya di sini untuk menghemat tempat]
...
local function resetTool()
    ...
    equipped = false
    -- Tidak perlu panggil equipToolOnce() di sini, akan dipanggil di forceResetFishing
end

local function doChargeAndRequest()
    -- NEW: Mengurangi jumlah pemanggilan Request Minigame yang tidak perlu
    safeInvoke(RFChargeFishingRod, 2)
    
    -- Pemanggilan Request Minigame yang agresif
    safeInvoke(RFRequestFishingMinigameStarted, -1.25, 1)
    task.wait(0.01) -- Waktu tunggu yang sangat kecil
    safeInvoke(RFRequestFishingMinigameStarted, -1.25, 1)
end
...
local function forceResetFishing()
    ...
    -- Mencoba menyinkronkan ulang tool
    task.wait(0.2)
    equipToolOnce() 
    task.wait(0.2) 
    ...
    -- NEW: Memberi notifikasi agar pengguna tahu jika ada reset
    -- showNotification("🎣 Auto Fishing", "Force Reset Terjadi, Siklus Dimulai Ulang.")
end

-- NEW: Mempercepat dan Mengagresifkan Anti-Stuck
local function fishCheckLoop()
    local retryCount = 0
    
    while running and fishCheckEnabled do
        local currentTime = tick()
        -- Periksa jika tidak ada ikan tertangkap dalam 3 detik
        if currentTime - lastFishTime >= 3 and lastFishTime > 0 then 
            retryCount = retryCount + 1
            forceResetFishing()
            
            -- Jika gagal terus setelah 5 kali reset, berikan peringatan
            if retryCount >= 5 then
                showNotification("❌ Auto Fishing", "Gagal melakukan reset berulang kali. Coba nonaktifkan dan aktifkan kembali!")
                retryCount = 0 -- Reset counter
            end
        else
            retryCount = 0
        end
        task.wait(0.5) -- Cek setiap 0.5 detik (lebih sering)
    end
end
...
local function equipToolLoop()
    while running do
        safeFire(REEquipToolFromHotbar, 1)
        task.wait(1) -- Lebih sering equip ulang
    end
end

-- NEW: Fungsi periodicResetLoop yang sedikit lebih sering (30 menit)
local function periodicResetLoop()
    while running do
        task.wait(1800) -- Reset setiap 30 menit
        if running then
            forceResetFishing() -- Gunakan forceResetFishing
            lastResetTime = tick()
        end
    end
end
...
local function setupFishCaughtHandler()
    ...
    fishCaughtConnection = REFishCaught.OnClientEvent:Connect(function(fishId, weightData, itemData, isNew)
        ...
        if running then
            -- NEW: Hapus task.wait(0.08) untuk membuatnya INSTANT
            doChargeAndRequest() 
        end
    end)
end
...
local function fishingCycle()
    ...
    task.wait(0.2) -- Waktu tunggu inisial yang lebih pendek
    
    -- Mulai siklus memancing pertama
    equipToolOnce()
    task.wait(0.1)
    doChargeAndRequest()  
...
autoFishingToggle = Auto:Toggle({
    ...
    Type = "Toggle",
    Desc = "INSTANT FISHING - Siklus lebih agresif dan Anti Stuck diperbaiki.",
    Default = false,
    Callback = function(state) 
        running = state
        ...
        if running then
            ...
            showNotification("🎣 Auto Fishing", "Auto Fishing dimulai dengan siklus agresif.")
        else
