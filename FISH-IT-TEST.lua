...
-- Tambahkan fungsi pencarian remote yang lebih umum:
local function findRemote(name)
    local foundRemote = nil
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    
    -- Coba cari di path yang lama (untuk kompatibilitas)
    local path = ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net
    if path and path:FindFirstChild(name) then
        return path[name]
    end
    
    -- Coba cari di seluruh ReplicatedStorage (Pencarian brute-force)
    for _, descendant in pairs(ReplicatedStorage:GetDescendants()) do
        if descendant:IsA("RemoteEvent") or descendant:IsA("RemoteFunction") then
            if string.find(descendant.Name, name, 1, true) then
                foundRemote = descendant
                break
            end
        end
    end
...
-- Ambil Remote Events menggunakan fungsi pencarian baru
local REEquipToolFromHotbar = findRemote("EquipToolFromHotbar") -- Biasanya RE/EquipToolFromHotbar
local RFChargeFishingRod = findRemote("ChargeFishingRod") -- Biasanya RF/ChargeFishingRod
local RFRequestFishingMinigameStarted = findRemote("RequestFishingMinigameStarted") -- Biasanya RF/RequestFishingMinigameStarted
local REFishingCompleted = findRemote("FishingCompleted") -- Biasanya RE/FishingCompleted
local REUnequipToolFromHotbar = findRemote("UnequipToolFromHotbar") -- Biasanya RE/UnequipToolFromHotbar
local RFCancelFishingInputs = findRemote("CancelFishingInputs") -- Biasanya RF/CancelFishingInputs
local REFishCaught = findRemote("FishCaught") -- Biasanya RE/FishCaught atau RE/ObtainedNewFishNotification
...
-- [Fungsi safeFire, safeInvoke, dan showNotification tetap sama]
...
local function equipToolOnce()
    if not equipped then
        -- NEW: Hanya panggil sekali untuk meminimalisir spam event Equip
...
local function doChargeAndRequest()
    -- NEW: Hanya panggil Charge dan Request SATU KALI dengan jeda minimal
    safeInvoke(RFChargeFishingRod, 2)
    task.wait(0.01) -- Jeda minimal untuk sinkronisasi
    safeInvoke(RFRequestFishingMinigameStarted, -1.25, 1)
end

local function forceResetFishing()
    -- Panggil Cancel Input terlebih dahulu
    for i = 1, 2 do
    ...
    -- Mencoba menyinkronkan ulang tool dan memulai siklus
    task.wait(0.5) -- Jeda yang lebih jelas sebelum re-equip
    equipToolOnce() 
    task.wait(0.1) 
...
-- NEW: Anti-Stuck loop lebih fokus pada waktu
local function fishCheckLoop()
    while running and fishCheckEnabled do
        local currentTime = tick()
        -- Jika tidak ada ikan tertangkap dalam 2.5 detik (Waktu yang lebih akurat)
        if currentTime - lastFishTime >= 2.5 and lastFishTime > 0 then 
            forceResetFishing()
        end
...
-- NEW: Fungsi spamCompletedLoop TIDAK perlu dimodifikasi
...
-- NEW: Fungsi equipToolLoop (untuk mencegah tool ter-*unequip*)
local function equipToolLoop()
    while running do
        safeFire(REEquipToolFromHotbar, 1)
        task.wait(2) -- Jeda 2 detik agar tidak terlalu spam
    end
}

-- NEW: Fungsi periodicResetLoop (30 menit)
local function periodicResetLoop()
    while running do
        task.wait(1800) -- Reset setiap 30 menit
        if running then
            forceResetFishing() 
            lastResetTime = tick()
        end
...
-- NEW: Handler Ikan Tertangkap (Inti dari Instant Fish)
local function setupFishCaughtHandler()
    if fishCaughtConnection then
        fishCaughtConnection:Disconnect()
    end
    
    fishCaughtConnection = REFishCaught.OnClientEvent:Connect(function(fishId, weightData, itemData, isNew)
        lastFishTime = tick()
        
        if running then
            -- NEW: PANGGILAN INSTANT TANPA JEDA APAPUN
            safeFire(REFishingCompleted) -- Panggil Completed SEBELUM Charge/Request
...
local function fishingCycle()
    ...
    setupFishCaughtHandler() -- Pasang handler
    ...
    -- Loop utama hanya memastikan status 'running' tetap terjaga
    while running do
        task.wait(1) 
    end
...
autoFishingToggle = Auto:Toggle({
    Title = "Auto Fishing (Instant Fish V2)", 
    Type = "Toggle",
    Desc = "Perbaikan siklus memancing yang lebih stabil dan cepat.",
    Default = false,
    Callback = function(state) 
        running = state
        ...
        if running then
            ...
            showNotification("🎣 Auto Fishing", "Auto Fishing V2 dimulai. Semoga lebih stabil.")
        else
            ...
            showNotification("🎣 Auto Fishing", "Auto Fishing dihentikan.")
        end
    end
})
