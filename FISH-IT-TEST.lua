...
-- Default variables
local spamDelay = 0.01       -- Delay between fishing spam
local resetInterval = 300     -- Reset tool interval (seconds)
...
Auto:Slider({
    Title = "Reset Timer (sec)",
    Min = 60,
    Max = 600,
    Default = resetInterval,
    Callback = function(value)
        resetInterval = value
    end
})
...
-- Spam completed loop with adjustable delay
...
-- Periodic reset loop with adjustable interval
...
autoFishingToggle = Auto:Toggle({
    Title = "Auto Fishing",
    Type = "Toggle",
    Desc = "INSTANT FISHING - WITH ANTI STUCK SYSTEM + Adjustable Speed & Reset",
    Default = false,
    Callback = function(state)
        running = state
