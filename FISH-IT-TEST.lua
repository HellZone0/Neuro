...
-- Fish caught handler (Double Cast)
REFishCaught.OnClientEvent:Connect(function(fishName,fishData)
    lastFishTime = tick()
    if running then
        task.spawn(function()
            -- Lempar pertama selesai → langsung lempar kedua (Double Cast)
            safeFire(REFishingCompleted)
            task.wait(0.002 + math.random()*0.003) -- micro-delay
            doChargeAndRequest() -- langsung lempar lagi
            
            -- Lempar kedua selesai → catch kedua
            safeFire(REFishingCompleted)
            task.wait(0.002 + math.random()*0.003)
            doChargeAndRequest() -- siap lempar berikutnya
        end)
    end
end)
...
autoFishingToggle = Auto:Toggle({
    Title="Auto Fishing",
    Type="Toggle",
    Desc="Hybrid Ultra Double Cast Instant Fishing",
    Default=false,
    Callback=function(state)
        running = state
        if running then
