...
-- Spam completed loop (Instant catch)
local function spamCompletedLoop()
    while running do
        safeFire(REFishingCompleted)
        task.wait(0.08 + math.random()*0.04) -- random delay agar stabil
    end
end
...
-- Periodic server rest (every 5 minutes)
...
autoFishingToggle=Auto:Toggle({
    Title="Auto Fishing",
    Type="Toggle",
    Desc="Stable Instant Fishing with Anti-Stuck",
    Default=false,
    Callback=function(state)
        running=state
        if running then
