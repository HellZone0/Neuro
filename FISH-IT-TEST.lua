...
-- Charge rod & request fishing minigame
...
local function setupFishCaughtHandler()
    REFishCaught.OnClientEvent:Connect(function(fishName, fishData)
        ...
        if running then
            task.spawn(function()
                task.wait(0.15) -- small delay to avoid spam
                doChargeAndRequest()
            end)
        end
    end)
...
local function fishCheckLoop()
    ...
    while running and fishCheckEnabled do
        ...
        if timeSinceLastFish >= 10 then
            retryCount = retryCount + 1
            forceResetFishing()
            showNotification("Auto Fishing", "No fish detected, forcing reset...")

            if retryCount >= maxRetries then
                retryCount = 0
                resetTool()
                showNotification("Auto Fishing", "Tool reset due to inactivity")
            end
        else
            retryCount = 0
        end
...
-- Spam fishing completed to speed up fish
local function spamCompletedLoop()
    while running do
        safeFire(REFishingCompleted)
        task.wait(0.5) -- speed adjustable
    end
end

-- Equip tool loop to maintain tool equipped
...
-- Periodic tool reset every 5 minutes
local function periodicResetLoop()
    while running do
        ...
        if running then
            resetTool()
            lastResetTime = tick()
            showNotification("Auto Fishing", "Periodic tool reset executed")
        end
    end
end
...
autoFishingToggle = Auto:Toggle({
    ...
    Callback = function(state) 
        ...
        if running then
            ...
            showNotification("Auto Fishing", "Started")
        else
            safeInvoke(RFCancelFishingInputs)
            equipped = false
            fishCheckEnabled = false
