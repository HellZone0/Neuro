...
-- Spam completed loop (ultra-agresif)
...
-- Fish caught handler (multi-catch 2–3 ikan per lemparan)
REFishCaught.OnClientEvent:Connect(function(fishName,fishData)
    lastFishTime=tick()
    if running then
        task.spawn(function()
            local multiCatch = math.random(2,3) -- 2-3 ikan per lemparan
            for i=1,multiCatch do
                safeFire(REFishingCompleted)
                task.wait(0.005 + math.random()*0.005)
            end
            -- langsung charge & request lemparan berikutnya
            doChargeAndRequest()
        end)
    end
end)
...
autoFishingToggle=Auto:Toggle({
    Title="Auto Fishing",
    Type="Toggle",
    Desc="Hybrid Ultra-Multi-Catch (2–3 ikan/lemparan)",
    Default=false,
    Callback=function(state)
        running=state
        if running then
