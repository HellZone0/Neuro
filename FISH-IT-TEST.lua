...
local Section = Auto:Section({
    Title = "Ultimate Instant Fishing",
})
...
-- Fish caught handler (Double Cast + Pseudo Multi-Catch)
REFishCaught.OnClientEvent:Connect(function(fishName,fishData)
    lastFishTime = tick()
    if running then
        task.spawn(function()
            local pseudoCount = math.random(2,3) -- pseudo multi-catch per strike
            for i=1,pseudoCount do
                -- Double Cast → 2 lemparan cepat
                safeFire(REFishingCompleted)
...
autoFishingToggle = Auto:Toggle({
    Title="Ultimate Hybrid Extreme Fishing",
    Type="Toggle",
    Desc="Ultra Fast + Double Cast + Pseudo Multi-Catch + Anti-Stuck",
    Default=false,
    Callback=function(state)
        running = state
        if running then
