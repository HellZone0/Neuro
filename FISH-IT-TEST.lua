...
-- Anti-stuck / adaptive loop
local function fishCheckLoop()
    while running do
        local idleTime = tick() - lastFishTime
        if idleTime > 8 then
            forceResetFishing()
        end
        task.wait(0.4 + math.random()*0.2) -- adaptive micro-delay
    end
end

-- Fish caught handler (Pseudo Multi-Catch Queue)
REFishCaught.OnClientEvent:Connect(function(fishName,fishData)
    lastFishTime = tick()
    if running then
        task.spawn(function()
            -- Queue fishing beberapa kali secepat mungkin
            local queueCount = math.random(2,3) -- pseudo multi-catch
            for i=1,queueCount do
                safeFire(REFishingCompleted)
                task.wait(0.003 + math.random()*0.003)
                doChargeAndRequest()
            end
        end)
    end
...
-- Periodic server rest (5 menit)
local function periodicServerRest()
    while running do
        task.wait(300)
        safeInvoke(RFCancelFishingInputs)
...
autoFishingToggle = Auto:Toggle({
    Title="Auto Fishing",
    Type="Toggle",
    Desc="Hybrid Ultra-Pseudo Multi-Catch Instant Fishing",
    Default=false,
    Callback=function(state)
        running = state
        if running then
