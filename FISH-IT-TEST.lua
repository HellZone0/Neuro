...
-- Default Delay / Reel Speed
local delaySpeed = 0.05    -- Delay antar lemparan umpan
...
-- Spam Completed Loop (Reel / Catch Speed)
local function spamCompletedLoop()
    while running do
        safeFire(REFishingCompleted)
        task.wait(reelSpeed) -- baca reelSpeed langsung
    end
end
...
local function setupFishCaughtHandler()
    REFishCaught.OnClientEvent:Connect(function(fishName,fishData)
        lastFishTime = tick()
        if running then
            task.wait(delaySpeed) -- tunggu delaySpeed sebelum lempar ulang
            doChargeAndRequest()
        end
    end)
end
...
-- Input Box Delay Speed (umpan) max 10 detik
Auto:Input({
...
-- Input Box Reel Speed (timer / catch) min 0.001 detik
Auto:Input({
