...
-- Default Delay / Reset
local spamDelay = 0.01      -- Delay antara spam fishing
local resetInterval = 300    -- Interval reset tool (detik)
...
-- Spam Completed Loop (gunakan spamDelay langsung setiap iterasi)
local function spamCompletedLoop()
    while running do
        safeFire(REFishingCompleted)
        task.wait(spamDelay) -- baca nilai langsung
    end
end
...
-- Periodic Reset Loop (gunakan resetInterval langsung setiap iterasi)
...
-- Input Box Delay Speed (detik) dengan max 10
Auto:Input({
    Title = "Delay Speed (detik)",
    Placeholder = tostring(spamDelay),
    Callback = function(value)
        local num = tonumber(value)
        if num and num > 0 then
            if num > 10 then num = 10 end
            spamDelay = num -- update langsung
        end
    end
})

-- Input Box Reset Timer (detik) dengan min 10
Auto:Input({
    Title = "Reset Timer (detik)",
    Placeholder = tostring(resetInterval),
    Callback = function(value)
        local num = tonumber(value)
        if num and num >= 10 then
            resetInterval = num -- update langsung
        end
    end
})
