...
local function createSecretEmbed(fishName, playerName, weight, mutationStatus, fishPrice)
    return {
        ...
        thumbnail = {
            url = "https://cdn.discordapp.com/attachments/1422181713114824765/1428652693907570741/38d29524-906d-49a7-893f-044124ce3668.jpg?ex=68f49981&is=68f34801&hm=cb46d578f615b66ddacb515f2ae45070cc2f90b520684e247efd5bde4a4a8b6e&"
        },
        fields = {
            {
                name = "**Fisherman**",
...
local function createNormalEmbed(fishName, playerName, tierName, tierColor, tierEmoji, weight, mutationStatus, fishPrice)
    return {
        ...
        footer = {
            text = "HellZone Community • " .. os.date("%m/%d/%Y %I:%M %p")
        },
        timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
    }
end
...
local function sendFishWebhook(fishId, weightData, itemData, isNew)
    ...
    if webhookUrl and webhookUrl ~= "YOUR_WEBHOOK_URL_HERE" then
        ...
        local payload = {
            ...
            avatar_url = "https://cdn.discordapp.com/attachments/1422181713114824765/1428652693907570741/38d29524-906d-49a7-893f-044124ce3668.jpg?ex=68f49981&is=68f34801&hm=cb46d578f615b66ddacb515f2ae45070cc2f90b520684e247efd5bde4a4a8b6e&",
            embeds = { embed }
        }
        ...
        embeds = {
            {
                ...
                footer = {
                    text = "HellZoneCommunity • " .. os.date("%m/%d/%Y %I:%M %p")
                },
                timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
            }
        }
...
-- ======= Instant Fish Mode (Added: Normal / Ultra Fast / No Animation) =======
do
    -- mode state
    local InstantFishMode = "Normal" -- "Normal" | "Ultra Fast" | "No Animation"
    local instantRunning = false

    -- Ultra config (full instant spam)
    local ultraRunning = false
    local ultraErrorCount = 0
    local ultraErrorLimit = 12
    local ultraBaseDelay = 0.0 -- no deliberate wait, rely on jitter to avoid hard pattern
    local ultraJitterMax = 0.001 -- tiny jitter to vary timing
    local ultraConn = nil

    -- helper: jitter
    local function ultraJitter() return math.random() * ultraJitterMax end

    -- safe wrappers assumed present in main script: safeFire, safeInvoke
    -- fallback definitions if not present (very conservative)
    if not safeFire then
    ...
    -- Ultra: force reset quick
    local function ultraForceReset()
        pcall(function()
            if RFCancelFishingInputs then
                for i=1,2 do pcall(function() safeInvoke(RFCancelFishingInputs) end) end
            end
        end)
        pcall(function() if REUnequipToolFromHotbar then safeFire(REUnequipToolFromHotbar) end end)
        task.wait(0.06)
        pcall(function() if REEquipToolFromHotbar then safeFire(REEquipToolFromHotbar,1) end end)
        task.wait(0.06)
        pcall(function() if RFChargeFishingRod then safeInvoke(RFChargeFishingRod,2) end end)
        task.wait(0.02)
        pcall(function() if RFRequestFishingMinigameStarted then safeInvoke(RFRequestFishingMinigameStarted,-1.25,1) end end)
        lastFishTime = tick()
    end

    local function ultraDoChargeAndRequest()
        pcall(function() if RFChargeFishingRod then safeInvoke(RFChargeFishingRod,2) end end)
        task.wait(0.01 + ultraJitter()*0.5)
        pcall(function() if RFRequestFishingMinigameStarted then safeInvoke(RFRequestFishingMinigameStarted,-1.25,1) end end)
    end

    local function ultraSpamLoop()
        while ultraRunning do
            local ok = pcall(function() return safeFire(REFishingCompleted) end)
            if not ok then
                ultraErrorCount = ultraErrorCount + 1
            else
                ultraErrorCount = 0
            end

            if ultraErrorCount >= ultraErrorLimit then
                ultraErrorCount = 0
                ultraForceReset()
                task.wait(0.18 + math.random()*0.25)
            else
                task.wait(ultraBaseDelay + ultraJitter())
            end
        end
    end

    local function ultraSetupHandler()
        if not REFishCaught then return end
        if ultraConn and ultraConn.Connected then return end
        ultraConn = REFishCaught.OnClientEvent:Connect(function(...)
            lastFishTime = tick()
            if ultraRunning then
                task.wait(0.02 + ultraJitter()*0.5)
                pcall(ultraDoChargeAndRequest)
            end
        end)
    end

    local function ultraTeardown()
        if ultraConn then
            pcall(function() ultraConn:Disconnect() end)
            ultraConn = nil
        end
    end

    function StartUltraFish()
        if ultraRunning then return end
        ultraRunning = true
        ultraErrorCount = 0
        lastFishTime = tick()
        -- ensure normal autoFishingToggle is off
        pcall(function() if autoFishingToggle and autoFishingToggle.Set then autoFishingToggle:Set(false) end end)
        ultraSetupHandler()
        task.spawn(ultraSpamLoop)
        pcall(function() ultraDoChargeAndRequest() end)
        print("[FISH-IT] StartUltraFish() called")
    end

    function StopUltraFish()
        ultraRunning = false
        ultraErrorCount = 0
        ultraTeardown()
        print("[FISH-IT] StopUltraFish() called")
    end

    -- Normal wrappers to control original autoFishingToggle
    function StartNormalFish()
        -- ensure ultra off
        pcall(StopUltraFish)
        pcall(function() if autoFishingToggle and autoFishingToggle.Set then autoFishingToggle:Set(true) end end)
        print("[FISH-IT] StartNormalFish() called")
    end

    function StopNormalFish()
        pcall(function() if autoFishingToggle and autoFishingToggle.Set then autoFishingToggle:Set(false) end end)
        print("[FISH-IT] StopNormalFish() called")
    end

    function StartNoAnimFish()
        -- No Animation: try to cancel animations and use minimal requests (behaves like instant w/o visuals)
        pcall(StopUltraFish)
        pcall(StopNormalFish)
        -- attempt minimal single charge/request loop
        task.spawn(function()
            if instantRunning then return end
            instantRunning = true
            while instantRunning do
                pcall(function()
                    if RFChargeFishingRod then safeInvoke(RFChargeFishingRod,2) end
                    if RFRequestFishingMinigameStarted then safeInvoke(RFRequestFishingMinigameStarted,-1.25,1) end
                    safeFire(REFishingCompleted)
                end)
                task.wait(0.08)
            end
        end)
        print("[FISH-IT] StartNoAnimFish() called")
    end

    function StopNoAnimFish()
        instantRunning = false
        print("[FISH-IT] StopNoAnimFish() called")
    end

    -- Auto-reactivate Ultra after respawn if mode chosen
    pcall(function()
        if player and player.CharacterAdded then
            player.CharacterAdded:Connect(function()
                task.wait(1)
                if InstantFishMode == "Ultra Fast" then
                    task.wait(0.6)
                    pcall(StartUltraFish)
                end
            end)
        end
    end)

    -- Watchdog to keep ultra healthy
    task.spawn(function()
        while true do
            if ultraRunning then
                if tick() - lastFishTime > 6 then
                    ultraForceReset()
                end
            end
            task.wait(1)
    ...
    -- Create UI elements in Auto (Main tab) if available
    pcall(function()
        if Auto and Auto.Dropdown and Auto.Toggle then
            -- create a small section for Fishing modes if Section API exists
            local ok, section = pcall(function() return Auto:Section({Title = "Fishing"}) end)
            local fishingSection = (ok and section) or Auto

            -- Dropdown for mode
            fishingSection:Dropdown({
                Title = "Instant Fish Mode",
                Values = {"Normal", "Ultra Fast", "No Animation"},
                Value = "Normal",
                Multi = false,
                Callback = function(selected)
                    InstantFishMode = selected
                    if selected == "Normal" then
                        StopUltraFish()
                        StopNoAnimFish()
                        StartNormalFish()
                    elseif selected == "Ultra Fast" then
                        StopNoAnimFish()
                        StopNormalFish()
                        StartUltraFish()
                    else -- No Animation
            ...
            -- Toggle for enable/disable instant fish (independent)
            fishingSection:Toggle({
                Title = "Enable Instant Fish",
                Default = false,
                Callback = function(state)
                    if state then
                        if InstantFishMode == "Normal" then
                            StartNormalFish()
                        elseif InstantFishMode == "Ultra Fast" then
                            StartUltraFish()
                        else
                            StartNoAnimFish()
                        end
                    else
                        -- stop all modes safely
                        StopUltraFish()
                        StopNormalFish()
                        StopNoAnimFish()
                    end
            ...
            -- Fallback: create a small ScreenGui toggle if Auto UI not present
            if player and player:FindFirstChild("PlayerGui") then
                local okGui, _ = pcall(function()
                    local screenGui = Instance.new("ScreenGui")
                    screenGui.Name = "FISHIT_InstantFish_UI"
                    screenGui.ResetOnSpawn = false
                    screenGui.Parent = player:WaitForChild("PlayerGui")

                    local frame = Instance.new("Frame")
                    frame.Size = UDim2.new(0,220,0,90)
                    frame.Position = UDim2.new(0,8,0,160)
                    ...
                    modeLabel.Size = UDim2.new(0,84,0,22)
                    modeLabel.Position = UDim2.new(0,140,0,40)
                    ...
                    toggleBtn.MouseButton1Click:Connect(function()
                        if not instantRunning then
                            -- start according to currentMode
                            if currentMode == "Normal" then StartNormalFish() end
                            if currentMode == "Ultra Fast" then StartUltraFish() end
                            if currentMode == "No Animation" then StartNoAnimFish() end
                            instantRunning = true
                            toggleBtn.Text = "Disable Instant"
                        else
                            StopUltraFish()
                            StopNormalFish()
                            StopNoAnimFish()
                    ...
                    -- simple mode switch on click of modeLabel (cycles)
                    modeLabel.MouseButton1Click = nil
                    modeLabel.InputBegan:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.MouseButton1 then
                            if currentMode == "Normal" then currentMode = "Ultra Fast"
                            elseif currentMode == "Ultra Fast" then currentMode = "No Animation"
                            else currentMode = "Normal" end
                            modeLabel.Text = "Mode: "..currentMode
                            InstantFishMode = currentMode
                        end
                    end)
                end)
            end
...
-- ======= End Instant Fish Mode Integration =======
